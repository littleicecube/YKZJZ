...
public class DownloadZJZ {
	...
	public void testDown() {
		...
		String url="https://v.youku.com/v_show/id_XMzkxNzAzNDc1Mg==.html?spm=a2h0k.11417342.soresults.dplaybutton&s=492eed6ff9fe4c5f8f1f";
		line = "you-get -o violin "+url;
	...
	public void downLoad() {
		...
		sche.scheduleAtFixedRate(()->{
			...
			if(StringUtils.isNoneBlank(filePath)) {
				...
				if(file.exists()) {
					...
					String msg = "�ļ���С:"+(size.get()/1024)+",��ǰ��С:"+(len/1024)+"kb";
					sb.append(msg);
		...
		while(true) {
			try {
				if(isEnd.get()) {
					System.out.println(this.filePath+File.separator+this.fileName+"�������");
				...
				try {
					...
					while((len = is.read(bArr)) > 0) {
						...
						if(ss.contains("size") && ss.contains("bytes)") && ss.contains("(")) {
							...
							if(size.get() == 0 ) {
								throw new RuntimeException("�ļ����Ȼ�ȡʧ��:"+ss);
							}
						}
